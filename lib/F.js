...
F.FilterIterator.prototype.next = function() {
  ...
  /* This statement is reached when the collection is empty or if the collection
  ends in elements which fail the predicate. This was a conscios decision, so we
  don't have to forward-scan the iterator to see if there are any more valid valued. */
  return {
    done: value.done
  };
};
 ...
 */
F.DropIterator = function(iterator, count) {
  this._iter = iterator;
  this._count = count;
};

F.DropIterator.prototype.next = function() {
  /* Because the count property is mutated, this while will only happen
  the first time the drop method is called. */
  while (this._count-- > 0) {
    this._iter.next();
  }
 ...
 *  iterable
 */
F.DropWhileIterator = function(iterator, pred) {
  this._iter = iterator;
  this._pred = pred;
  this._dropped = false;
};
...
/**
 * This method is called the first time the "next" method is called, and it
 * drop the first elements until the predicate is no longer satisfied.
 */
F.DropWhileIterator.prototype.dropElements = function() {
  var value;
  ...
  /* This point is reached when no elements satisfy the predicate */
  return {
    done: true
  };
}

F.DropWhileIterator.prototype.next = function() {
  if (!this._dropped) {
    this._dropped = true;
...
F.ZipIterator.prototype.next = function() {
  ...
  /* If either the collections are empty, then just skip this zip */
...
F.ConcatIterator.prototype.next = function() {
  ...
  do {
    ...
    if (F.isEmptyValue(value) && this._iterators.length) {
...
F.Iterable.prototype.concat = function() {
...
F.Iterable.prototype.flatten = function() {
  return F(new F.ConcatIterator(
...
F.Iterable.prototype.zip = function(other) {
  return F(new F.ZipIterator(this._iter, F(other)
    ._iter));
};
...
F.Iterable.prototype.toMap = function() {
  var result = {},
    value;
...
F.Iterable.prototype.drop = function(count) {
  return F(new F.DropIterator(this._iter, count));
};
...
F.Iterable.prototype.dropWhile = function(pred) {
  return F(new F.DropWhileIterator(this._iter, pred));
};

F.Iterable.prototype.fold = function(func, startValue) {
  var value;
 ...
 */
F.Iterable.prototype.foreach = function(func) {
  var value,
    index = 0;
 ...
 */
F.Iterable.prototype.reverse = function() {
  var value,
    reversed = [];
