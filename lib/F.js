(function() {
  ...
  var ZipIterator = function(iterator, zipIterator) {
    ...
    /* Because values can be filtered, they need to be kept in cache
    until both values are undefined */
    this._value = {
      done: false
    };
    this._zipValue = {
  ...
  ZipIterator.prototype.next = function() {
    if (isEmptyValue(this._value)) {
      this._value = this._iter.next();
    }
    if (isEmptyValue(this._zipValue)) {
      this._zipValue = this._zipIter.next();
    }

    var ret = {
      done: this._value.done || this._zipValue.done,
      value: [this._value.value, this._zipValue.value]
    };

    if (isEmptyValue(this._value) || isEmptyValue(this._zipValue)) {
      /* If one of the values got filtered, then filter ourselfs */
      delete ret.value;
    } else {
      /* Otherwise clear the cache and move onto the next values */
      this._value.value = undefined;
      this._zipValue.value = undefined;
    }
