...
(function(root) {
  ...
  var F = function(iterator) {
    if (iterator instanceof Iterable) {
      ...
      return new Iterable(new FunctionGeneratorIterator(iterator));
    } else if (typeof iterator.length === "number") {
      return new Iterable(new ArrayIterator(iterator));
    } else {
      return new Iterable(new ObjectIterator(iterator));
    }
  };
  ...
  Iterable.prototype.drop = function(count) {
    return F(new FilterIterator(this._iter, P.not(P.count(count))));
  };
  ...
  Iterable.prototype.dropWhile = function(pred) {
    ...
    return F(new FilterIterator(this._iter, function(value) {
      if (dropped) {
        return true;
      } else if (!pred(value)) {
        dropped = true;
  ...
  Iterable.prototype.keys = function() {
    ...
    return F(new MapIterator(this._iter, function(pair) {
      return pair[0];
    }));
  };
  ...
  Iterable.prototype.min = function(lessThan) {
    ...
    return F(this._iter).max(lessThan);
  };
